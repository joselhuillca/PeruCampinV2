...
namespace MLearning.Droid.Views
{
	...
	public class MainView : MvxActionBarActivity, VerticalScrollViewPager.ScrollViewListenerPager
	{
		...
		private void iniMenu(){
			...
			_listTasksTop.DividerHeight = 0;
			_listTasksBotton = new ListView (this);
			_listTasksBotton.DividerHeight = 0;
		    ...
		    String adsImagesPath = "";
            if (this.ApplicationContext.Resources.GetText(val).Equals("hitec")) //spanish
		        adsImagesPath = "icons/fondo.png";
		    else
		        adsImagesPath = "icons/fondo_en.png";

            Drawable d = new BitmapDrawable (getBitmapFromAsset (adsImagesPath));
			...
			linearListTaskTop.ShowDividers = ShowDividers.None;
			...
			linearListTaskBotton.ShowDividers = ShowDividers.None;
		...
		public void initListTaskTop()
		{
			...
			home.Name = Resources.GetText(Resource.String.preparandoPartida);//"Home";
			fotos.Name = "Fotos";

			caminoCifras.Name = Resources.GetText(Resource.String.mejores50campamentos);//¿óo usar la guí?";
		...
		void _listTasksItemTop_Click (object sender, AdapterView.ItemClickEventArgs e)
		{
			...
			if (e.Position == 0)//home
			{
                //showHome ();
                showServicios();//COSTA
				//Toast.MakeText (this, Resources.GetText(Resource.String.preparandoPartida), ToastLength.Short).Show();
			}
			else if(e.Position == 1)//rutas
			{
				...
				//} else {
                    //showRutas ();
                    showCifras();//SIERRA
					//Toast.MakeText (this, Resources.GetText(Resource.String.mejores50campamentos), ToastLength.Short).Show();
                //}
			}
		...
		void _listTaskItemBotton_Click (object sender, AdapterView.ItemClickEventArgs e)
		{
			if (e.Position == 0)//servicios
			{
				...
				//}  else {
                    //showServicios ();
                    showFavoritos();//SELVA
				//Toast.MakeText (this, Resources.GetText(Resource.String.mismejoresCampamentos), ToastLength.Short).Show();
				//}
			}
			else if(e.Position == 1)//silvestre
			{
				...
				//Toast.MakeText (this, Resources.GetText(Resource.String.palabraExpertos), ToastLength.Short).Show();
               // }
			}
		...
		public void initListTaskBotton()
		{
			...
			guiaServicios.Name = Resources.GetText(Resource.String.palabraExpertos);//"La palabra del experto";
            guiaSilvestre.Name = Resources.GetText(Resource.String.mismejoresCampamentos);//"Preparando la partida";
		...
		public void showCurso(int index)
		{
			...
			if (vm.CirclesList == null) {
				...
				myHandler.Post(()=>{
					Toast.MakeText (this, Resources.GetText(Resource.String.cursosDescargando), ToastLength.Short).Show();
				} );
				return;
			}
			...
			switch (index) {
			case 0:
				lo._txtCursoN.Text = "¿óo usar la guí?";
				break;
			case 1:
				lo._txtCursoN.Text = Resources.GetText(Resource.String.preparandoPartida);
				break;
			case 2:
				lo._txtCursoN.Text = Resources.GetText(Resource.String.palabraExpertos);
				break;
			case 3:
				lo._txtCursoN.Text = Resources.GetText(Resource.String.mejorescampametos50) ;
				break;
		...
		public void showServicios()		{  lo.indice_navigator = 0; lo.isNotas = false; lo.isFavoritos = false; showCurso (1);lo.header.SetBackgroundDrawable (headersDR[1]);lo._contentScrollView_S2.SetBackgroundColor (Color.ParseColor ("#00FFFF"));}
		public void showGuiaSilvestre()	{  lo.indice_navigator = 3; lo.isNotas = false; lo.isFavoritos = false; showCurso(2);lo.header.SetBackgroundDrawable (headersDR[2]);lo._contentScrollView_S2.SetBackgroundColor (Color.ParseColor ("#74DF00"));}
		public void showCifras()		{ lo.indice_navigator = 1; lo.isNotas = false;  lo.isFavoritos = false;showCurso (3);lo.header.SetBackgroundDrawable (headersDR[3]);lo._contentScrollView_S2.SetBackgroundColor (Color.ParseColor ("#f5ac10"));}
		public void showFavoritos()		{
			lo.indice_navigator = 2;
		...
		void lo_item_click (object sender, EventArgs e)
		{
			...
			if ( this.lo.isFavoritos) {
				...
				int indexSection = item.content.CurrentSection;
				for (int i = 0; i < vm.LOsectionList.Count; i++)
				{
					if (vm.LOsectionList[i].id == item.content.CurrentSection)
						indexSection = i;
		...
		void playSound(object sender, EventArgs e)
		{
			if (!isOnline ()) {
				var myHandler = new Handler ();
				myHandler.Post(()=>{
					Toast.MakeText (this, "Sin Conexió", ToastLength.Short).Show();
				} );
				return;
			}
		...
		public void loadSection(){
			...
			if (vm.LOsectionList != null) 
			{
				...
				if (vm.LearningOjectsList.Count <= 0) { return;}
				MLearning.Core.ViewModels.MainViewModel.lo_by_circle_wrapper currentLearningObject = vm.LearningOjectsList [_currentUnidad];
				int circleID = currentLearningObject.lo.Circle_id;

				if ( this.lo.isFavoritos) {
                    this.lo.listFavorites = FavoritosItemManager.GetTasks();
                    int tam = this.lo.listFavorites.Count;
                    if (tam == 0 && this.lo.isFavoritos)
                    {
                        lo._mainSpace.RemoveAllViews();
						lo.mensajeFavoritos.RemoveAllViews();
						Toast.MakeText(this, Resources.GetText(Resource.String.listaVaciamismejoresCampamentos), ToastLength.Short).Show();
						...
						lo.mensajeFavoritos.AddView(txtTmp);
						...
						lo.mensajeFavoritos.AddView(txtTmp2);

						lo._mainSpace.AddView(lo.mensajeFavoritos);
